<?php
/*============================================================================*\
  Generic USB byte device handler
...
/*============================================================================*\
  ...
  Commands:  
  get    : return the value of the bit    
  set byte (in decimal)    : set 
  toggle : change the bit value       
...
*/
  ...
  foreach($command as $cmd){
    ...
    switch ($opr){
      ...
      default:
        if(!$unit_id) return ["error"=>"No unit ID given"];
        $response=$this->operate($unit_id,$opr,$data);
        break;
    }
  }
...
\*==========================================================================*/
private function operate($unit_id,$opr,$value){
  // Get device name
  $dev_name=$this->list_devices($unit_id);
  ...
  if($opr == "mask"){
    $data['mask'] = bindec($value) & 0xff;
    
  }elseif(in_array($opr,["set","toggle"])){
    if($opr == "toggle") $value = ~$value & 0xff;
    $data['state'] = $value;  
    $byte = ($data['mask'] ^ $value) & 0xff;
...
//printf("Writing %X\n", ord(chr(intval($data) & 0xff)));
  $rc=@file_put_contents($fn,chr(intval($data) & 0xff)); // Might return false on succes!
  if($rc<1){
